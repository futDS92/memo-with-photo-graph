...
function normalizeAnswer(value: string) { return value.toLowerCase().replace(/[\s\u200b.,!?()[\]{}:;/%]+/g, "").trim(); }
function answerMatches(input: string, answer: string) { const normalized = normalizeAnswer(input); return answer.split(/[|/]/).some((item) => normalizeAnswer(item) === normalized); }
...
export function App() {
  ...
  const totalAttempts = cards.reduce((sum, card) => sum + (card.correctCount || 0) + (card.incorrectCount || 0), 0);
  ...
  useEffect(() => {
    ...
    let pinchDistance = 0;
    const distance = () => { const points = [...graphPointers.current.values()]; return points.length === 2 ? Math.hypot(points[0].x - points[1].x, points[0].y - points[1].y) : 0; };
    const onDown = (event: PointerEvent) => { canvas.setPointerCapture(event.pointerId); graphPointers.current.set(event.pointerId, { x: event.clientX, y: event.clientY }); if (graphPointers.current.size === 1) dragStart = { x: event.clientX, y: event.clientY, panX: graphPan.x, panY: graphPan.y }; else pinchDistance = distance(); };
    const onMove = (event: PointerEvent) => { if (!graphPointers.current.has(event.pointerId)) return; graphPointers.current.set(event.pointerId, { x: event.clientX, y: event.clientY }); if (graphPointers.current.size === 2) { const nextDistance = distance(); if (pinchDistance > 0) setGraphZoom((zoom) => Math.max(.8, Math.min(1.4, Number((zoom * (nextDistance / pinchDistance)).toFixed(2))))); pinchDistance = nextDistance; } else if (graphPointers.current.size === 1) setGraphPan({ x: dragStart.panX + event.clientX - dragStart.x, y: dragStart.panY + event.clientY - dragStart.y }); };
    const onUp = (event: PointerEvent) => { graphPointers.current.delete(event.pointerId); if (graphPointers.current.size < 2) pinchDistance = 0; };
    ...
    return () => { canvas.removeEventListener("pointerdown", onDown); canvas.removeEventListener("pointermove", onMove); canvas.removeEventListener("pointerup", onUp); canvas.removeEventListener("pointercancel", onUp); graphPointers.current.clear(); };
  }, [view]);
  useEffect(() => {
  ...
  const deleteCard = (id: string) => { const nextIds = studyIds.filter((studyId) => studyId !== id); setState((current) => ({ ...current, schemaVersion: 2, updatedAt: new Date().toISOString(), words: current.words.filter((card) => card.id !== id), relations: current.relations.filter((relation) => relation.fromWordId !== id && relation.toWordId !== id) })); setStudyIds(nextIds); setStudyIndex((index) => Math.min(index, Math.max(nextIds.length - 1, 0))); if (nextIds.length) saveStudySession({ mode, ids: nextIds, index: Math.min(studyIndex, nextIds.length - 1) }); else saveStudySession(null); setGraphFocusId((focusId) => focusId === id ? "all" : focusId); setSelectedId(null); setSheet(null); notify("카드를 삭제했어요"); };
  const editCard = (card: Word) => { setEditingId(card.id); setSheet("edit"); };
  const removePhoto = (id: string) => { setState((current) => ({ ...current, schemaVersion: 2, updatedAt: new Date().toISOString(), words: current.words.map((card) => card.id === id ? { ...card, photo: "" } : card) })); setPhotoOpen(false); notify("사진을 삭제했어요"); };
  const updateCard = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const card = cards.find((item) => item.id === editingId); if (!card) return; const data = new FormData(event.currentTarget); const term = String(data.get("question") || "").trim(); const definition = String(data.get("answer") || "").trim(); if (!term || !definition) return; if (cards.some((item) => item.id !== card.id && item.term.trim().toLowerCase() === term.toLowerCase())) { notify("같은 문제가 이미 있어요"); return; } const file = data.get("photo"); let photo = card.photo; if (file instanceof File && file.size) { try { photo = await photoUrl(file); } catch { notify("사진을 처리하지 못했어요"); return; } } setState((current) => ({ ...current, schemaVersion: 2, updatedAt: new Date().toISOString(), words: current.words.map((item) => item.id === card.id ? { ...item, term, definition, photo, pos: String(data.get("subject") || item.pos || "기타"), example: String(data.get("chapter") || item.example || "기본"), memo: String(data.get("memo") || ""), tags: String(data.get("tags") || "").split(",").map((tag) => tag.trim()).filter(Boolean) } : item) })); setSheet("detail"); notify("카드를 수정했어요"); };
  ...
  const chooseAnswer = (choice: string) => { if (!currentCard) return; if (answerMatches(choice, currentCard.definition)) { setSelectedChoice(choice); setRevealed(true); } else { setSelectedChoice(choice); setState((current) => ({ ...current, schemaVersion: 2, updatedAt: new Date().toISOString(), words: current.words.map((card) => card.id === currentCard.id ? { ...card, incorrectCount: (card.incorrectCount || 0) + 1 } : card) })); notify("Not quite — try again"); } };
  ...
  return <main className="study-app">
    {photoOpen && selected?.photo && <div className="photo-lightbox" role="dialog" aria-label="사진 크게 보기" onClick={() => setPhotoOpen(false)}><img src={selected.photo} alt="확대 사진" /><button type="button" onClick={() => setPhotoOpen(false)} aria-label="닫기">×</button></div>}
    {sheet === "edit" && selected && <div className="study-sheet"><div className="sheet-dim" onClick={() => setSheet("detail")} /><div className="study-sheet-panel"><div className="sheet-grab" /><div className="sheet-title"><div><p>카드 편집</p><h2>내용을 수정하세요</h2></div><button type="button" onClick={() => setSheet("detail")}><Icon name="close" /></button></div><form className="card-form" onSubmit={updateCard}><label>문제<input name="question" defaultValue={selected.term} autoFocus required /></label><label>정답<textarea name="answer" rows={4} defaultValue={selected.definition} required /></label><label className="photo-input">사진<input name="photo" type="file" accept="image/*" /></label><div className="form-grid"><label>과목<input name="subject" defaultValue={selected.pos} /></label><label>단원<input name="chapter" defaultValue={selected.example} /></label></div><label>태그<input name="tags" defaultValue={selected.tags.join(", ")} /></label><label>암기 메모<textarea name="memo" rows={2} defaultValue={selected.memo} /></label><button className="known wide" type="submit">저장</button></form></div></div>}
    {view === "stats" && <section className="study-content stats-page"><div className="study-page-title"><div><p>학습 기록</p><h1>통계</h1></div></div><div className="stats-hero"><strong>{accuracy}%</strong><span>전체 정답률</span><small>{totalAttempts ? `${totalAttempts}회 풀이` : "아직 기록이 없어요"}</small></div><div className="stats-grid"><div><strong>{cards.length}</strong><small>전체 카드</small></div><div><strong>{mastered}</strong><small>익힌 카드</small></div><div><strong>{dueCards.length}</strong><small>오늘 복습</small></div><div><strong>{mistakeCards.length}</strong><small>오답 카드</small></div></div><div className="stats-subjects"><strong>과목별 카드</strong>{subjects.slice(1).map((item) => <div key={item}><span>{item}</span><small>{cards.filter((card) => cardSubject(card) === item).length}장 · {cards.filter((card) => cardSubject(card) === item && isDue(card)).length}장 복습</small></div>)}</div></section>}
    {view === "graph" && <div className="graph-controls"><select value={graphSubject} onChange={(event) => { setGraphSubject(event.target.value); setGraphFocusId("all"); }} aria-label="과목 필터">{graphSubjects.map((item) => <option value={item} key={item}>{item === "All" ? "전체 과목" : item}</option>)}</select><select value={graphFocusId} onChange={(event) => setGraphFocusId(event.target.value)} aria-label="중심 개념"><option value="all">전체 개념</option>{cards.filter((card) => graphSubject === "All" || cardSubject(card) === graphSubject).map((card) => <option value={card.id} key={card.id}>{card.term}</option>)}</select><div className="graph-zoom"><button type="button" onClick={() => setGraphZoom((zoom) => Math.max(.8, Number((zoom - .1).toFixed(1))))} aria-label="축소">−</button><span>{Math.round(graphZoom * 100)}%</span><button type="button" onClick={() => setGraphZoom((zoom) => Math.min(1.4, Number((zoom + .1).toFixed(1))))} aria-label="확대">＋</button></div><svg className="graph-markers" aria-hidden="true"><defs><marker id="graph-arrow" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 z" /></marker></defs></svg></div>}
    ...
    {sheet === "add" && <div className="study-sheet"><div className="sheet-dim" onClick={() => setSheet(null)} /><div className="study-sheet-panel"><div className="sheet-grab" /><div className="sheet-title"><div><p>NEW CARD</p><h2>Create a study card</h2></div><button type="button" onClick={() => setSheet(null)}><Icon name="close" /></button></div><form className="card-form" onSubmit={addCard}><label>Front · question<input name="question" autoFocus placeholder="e.g. What is the condition for 3NF?" required /></label><label>Back · answer<textarea name="answer" rows={4} placeholder="Write the answer and explanation" required /></label><label className="photo-input">사진<input name="photo" type="file" accept="image/*" /></label><label className="memory-first">Memory note<textarea name="memo" rows={3} placeholder="What should you remember from this photo?" /></label><details className="advanced-fields"><summary>More card options</summary><div className="form-grid"><label>Subject<input name="subject" placeholder="Database" /></label><label>Chapter<input name="chapter" placeholder="Data Modeling" /></label></div><div className="form-grid"><label>Type<select name="type"><option value="concept">Concept</option><option value="formula">Formula</option><option value="case">Case</option><option value="multiple-choice">Multiple choice</option><option value="cloze">Cloze</option></select></label><label>Tags<input name="tags" placeholder="key, memorize" /></label></div><label>Choices · comma separated<input name="choices" placeholder="Option A, Option B, Option C" /></label></details><button className="known wide" type="submit">Save card</button></form></div></div>}

    {sheet === "detail" && selected && <div className="study-sheet"><div className="sheet-dim" onClick={() => setSheet(null)} /><div className="study-sheet-panel"><div className="sheet-grab" /><div className="sheet-title"><div><p>{cardSubject(selected)}</p><h2>Card details</h2></div><button type="button" onClick={() => setSheet(null)}><Icon name="close" /></button></div>{selected.photo && <div className="detail-photo-wrap"><button type="button" className="detail-photo-button" onClick={() => setPhotoOpen(true)}><img className="detail-photo" src={selected.photo} alt="Card visual" /></button><button type="button" className="photo-remove" onClick={() => removePhoto(selected.id)}>사진 삭제</button></div>}<div className="detail-card"><p>Front</p><h3>{selected.term}</h3><hr /><p>Back</p><strong>{selected.definition}</strong>{selected.memo && <div className="detail-note"><span>암기 메모</span><small>{selected.memo}</small></div>}</div><div className="detail-stats"><span>Correct {selected.correctCount || 0}</span><span>Mistakes {selected.incorrectCount || 0}</span><span>Level {selected.reviewLevel || 0}</span></div><div className="detail-actions"><button className="secondary-action" type="button" onClick={() => editCard(selected)}>수정</button><button className="danger-action" type="button" onClick={() => { if (window.confirm("이 카드를 삭제할까요?")) deleteCard(selected.id); }}>삭제</button></div><button className="known wide" type="button" onClick={() => { setSheet(null); setStudyIds([selected.id]); setStudyIndex(0); setRevealed(false); setClozeInput(""); setView("study"); }}>Study this card</button></div></div>}
